...
import { Instagram, Linkedin, Link2, Send, MessageCircle, X, ArrowUpRight, ArrowLeft, Volume2, VolumeX, ChevronLeft, ChevronRight } from 'lucide-react';
...
function MainApp() {
  ...
  const { theme, setTheme, translations, portfolioData, pressData, labData, collaboratorsData, contact } = useSiteStore();
  ...
  return (
    <motion.div 
      ...
      className="relative min-h-screen font-sans selection:bg-[#fe0000] selection:text-white overflow-x-hidden"
    >
      <Helmet>
      ...
      <header className="fixed top-0 w-full p-6 md:p-8 flex justify-between items-center z-40 mix-blend-difference text-white">
        <div className="flex items-center gap-4">
          <img src="https://i.ibb.co/6cFB7Hqz/1.jpg" alt="COYORA Logo" className="h-5 w-auto object-contain" loading="lazy" />
            ...
            >
              {labData.map((item, idx) => (
                ...
                >
                  ...
                  <img src={item.image} alt={translate(item.title)} className="w-full h-full object-cover grayscale opacity-60 group-hover:grayscale-0 group-hover:opacity-100 group-hover:scale-105 transition-all duration-700 transform-gpu will-change-transform" loading="lazy" />
            ...
            <form action={`https://formspree.io/f/${contact.formspreeId}`} method="POST" className="w-full max-w-2xl text-left mb-24 flex flex-col gap-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <select name="project_type" required defaultValue="" className="bg-transparent border-b border-[var(--color-border)] py-4 font-mono text-xs tracking-widest focus:outline-none focus:border-[#fe0000] transition-colors text-[var(--color-text)] placeholder:text-[var(--color-muted)] appearance-none cursor-pointer">
                  <option value="" disabled>{t.project_type}</option>
                ...
                <div className="flex flex-col md:flex-row gap-12 md:gap-24 items-center">
                  <div className="w-full md:w-2/5 relative group">
                    ...
                    <div className="aspect-[3/4] bg-[var(--color-surface)] overflow-hidden border border-[var(--color-border)] relative">
                      ...
                      <img src="https://i.ibb.co/pvwdGxYx/ADY05299.jpg" alt="Ramazan Habibov" className="w-full h-full object-cover grayscale opacity-70 group-hover:opacity-100 group-hover:scale-105 transition-all duration-1000 transform-gpu will-change-transform" loading="lazy" />
                    </div>
                    ...
                    <div className="flex flex-col md:flex-row gap-12 mb-16">
                      <div className="w-full md:w-1/2">
                        <div className="aspect-square bg-[var(--color-surface)] border border-[var(--color-border)] overflow-hidden relative group">
                          <div className="absolute inset-0 bg-[#fe0000]/20 opacity-0 group-hover:opacity-100 transition-opacity duration-700 z-10 mix-blend-overlay" />
                          <img src={lab.image} alt={translate(lab.title)} className="w-full h-full object-cover grayscale opacity-80 group-hover:opacity-100 group-hover:scale-105 transition-all duration-1000 transform-gpu will-change-transform" loading="lazy" />
                        </div>
                      </div>
                  ...
                  <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, ease: [0.16, 1, 0.3, 1] }}>
                    ...
                    {activeProject.images && activeSection !== 'web' && (
                      <div className="bg-[var(--color-border)] border border-[var(--color-border)]">
                        <div className="flex flex-col md:flex-row md:overflow-x-auto md:snap-x md:snap-mandatory hide-scrollbar gap-px">
                          {activeProject.images.map((img, idx) => (
                            <div key={`img-${idx}`} className="w-full md:flex-[0_0_50%] md:snap-center min-w-0 overflow-hidden bg-[var(--color-bg)] relative group">
                              ...
                              <img src={img} alt="Work" className="w-full h-auto object-cover grayscale opacity-80 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-700 pointer-events-none" loading="lazy" />
                            </div>
                          ))}
                        </div>
                      </div>
...
export default function App() {
  return (
    <Routes>
      <Route path="/apply/:type/:schemaId" element={<StandaloneForm />} />
