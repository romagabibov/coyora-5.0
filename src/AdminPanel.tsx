...
import { collection, getDocs, query, orderBy, addDoc, updateDoc, deleteDoc, doc, serverTimestamp, getDoc, setDoc, where, limit, startAfter } from 'firebase/firestore';
...
export default function AdminPanel({ onClose }: AdminPanelProps) {
  ...
  const [activeTab, setActiveTab] = useState<'theme' | 'branding' | 'translations' | 'about' | 'portfolio' | 'lab' | 'press' | 'contact' | 'forms' | 'applications' | 'capabilities' | 'collaborators' | 'admins'>('theme');
  ...
  const loadApplications = async (formId: string, loadMore = false) => {
    if (!formId) return;
    
    // Check cache first if not loading more
    if (!loadMore && appsCache[formId]) {
      setApplications(appsCache[formId].apps);
      setLastAppDoc(appsCache[formId].lastDoc);
      setHasMoreApps(appsCache[formId].hasMore);
      return;
    }
    ...
    try {
      ...
      setAppsCache(prev => ({
        ...prev,
        [formId]: { apps: updatedApps, lastDoc: lastVisible, hasMore }
      }));
    } catch (err) {
  ...
  const checkAdminRole = async (email: string | null) => {
    ...
    if (email === 'vnsbek@gmail.com' || email === 'coyoraofficial@gmail.com') {
      return 'superadmin';
    }
    try {
  ...
  const getOrderedFormData = (app: any) => {
    ...
    const schema = formSchemas.find(s => s.id === app.eventId);
    const orderedEntries: [string, any][] = [];
    const processedKeys = new Set<string>();

    if (schema && schema.fields) {
      schema.fields.forEach((field: any) => {
        const questionText = field.label || field.question; 
        if (app.formData[questionText] !== undefined) {
          orderedEntries.push([questionText, app.formData[questionText]]);
          processedKeys.add(questionText);
        }
      });
    }
                ...
                <div className="w-full md:w-48 border-b md:border-b-0 md:border-r border-[#e5e7eb] p-4 flex md:flex-col gap-2 font-mono text-sm overflow-x-auto md:overflow-x-visible whitespace-nowrap">
                  {(adminRole === 'superadmin') && (
                    <>
                      ...
                      <button onClick={() => setActiveTab('branding')} className={`text-left px-4 py-2 rounded ${activeTab === 'branding' ? 'bg-[#fe0000] text-white' : 'hover:bg-[#f9fafb]'}`}>Branding</button>
                      <button onClick={() => setActiveTab('admins')} className={`text-left px-4 py-2 rounded ${activeTab === 'admins' ? 'bg-[#fe0000] text-white' : 'hover:bg-[#f9fafb]'}`}>Admins</button>
                    </>
                  )}
                        ...
                        <div className="flex gap-2 flex-wrap items-center">
                          ...
                          <button
                            onClick={() => {
                              ...
                              if (window.confirm(`Are you sure you want to delete the "${selectedCategory}" category and all its projects?`)) {
                                useSiteStore.setState(state => {
                              ...
                              <div className="mt-6">
                                ...
                                <div className="space-y-3">
                                  ...
                                  <button 
                                    ...
                                    className="text-xs font-mono text-[#fe0000] flex items-center gap-1 hover:underline"
                                  >
