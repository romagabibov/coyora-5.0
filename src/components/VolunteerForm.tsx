...
import { addDoc, collection, serverTimestamp, query, where, getDocs, doc, setDoc } from 'firebase/firestore';
...
export function VolunteerForm({ eventId, eventName, type, fields, onSuccess, lang, translate }: VolunteerFormProps) {
  ...
  const handleNext = () => {
    ...
    if (currentStepData.required && (!value || (typeof value === 'string' && value.trim() === '') || (Array.isArray(value) && value.length === 0))) {
    ...
    if ((currentStepData.type === 'tel' || currentStepData.type === 'phone') && value) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (parseInt(captchaAnswer) !== captchaQuestion.num1 + captchaQuestion.num2) {
      showToast(translate("Incorrect math answer. Please try again."), true);
      return;
    }
    ...
    try {
      ...
      // Record submission for double-click prevention
      const now = Date.now();
  ...
  return (
    <div className="absolute inset-0 flex flex-col items-center justify-start p-6 md:p-8 bg-[var(--color-bg)] overflow-y-auto" data-lenis-prevent="true">
      <Toast message={toastMessage} isVisible={isToastVisible} onClose={() => setIsToastVisible(false)} isError={toastIsError} />
      <div className="w-full max-w-xl py-8 pb-32 flex-shrink-0">
          ...
          >
            ...
            {currentStep.type === 'textarea' ? (
              ...
              <select
                ...
                autoFocus
              >
                <option value="" disabled className="bg-[var(--color-bg)] text-[var(--color-text)]">Select an option</option>
                {(currentStep.options || []).map((opt: string, idx: number) => (
                  <option key={idx} value={opt} className="bg-[var(--color-bg)] text-[var(--color-text)]">{opt}</option>
                ))}
              </select>
            ) : currentStep.type === 'checkboxes' ? (
              <div className="space-y-3 w-full text-left">
                {(currentStep.options || []).map((opt: string, idx: number) => {
                  const currentValues = Array.isArray(formData[currentStep.id]) ? formData[currentStep.id] : [];
                  const isChecked = currentValues.includes(opt);
                  
                  return (
                    <label key={idx} className="flex items-center gap-3 p-3 border border-[var(--color-border)] rounded-lg cursor-pointer hover:border-[#fe0000] transition-colors">
                      <input
                        type="checkbox"
                        checked={isChecked}
                        onChange={(e) => {
                          if (e.target.checked) {
                            setFormData({ ...formData, [currentStep.id]: [...currentValues, opt] });
                          } else {
                            setFormData({ ...formData, [currentStep.id]: currentValues.filter((v: string) => v !== opt) });
                          }
                        }}
              ...
              <input
                type={currentStep.type === 'phone' ? 'tel' : currentStep.type}
                value={formData[currentStep.id] || ''}
                onChange={(e) => {
                  let val = e.target.value;
                  if (currentStep.type === 'tel' || currentStep.type === 'phone') {
            ...
            {step === formSteps.length - 1 && (
              <div className="mt-8 p-4 border border-[var(--color-border)] rounded-lg">
                <label className="block text-sm font-mono text-[var(--color-muted)] mb-2">
                  {translate('Please solve this to verify you are human:')} {captchaQuestion.num1} + {captchaQuestion.num2} = ?
                </label>
                <input
                  type="number"
                  value={captchaAnswer}
                  onChange={(e) => setCaptchaAnswer(e.target.value)}
                  className="w-full p-3 bg-transparent border border-[var(--color-border)] rounded-lg focus:border-[#fe0000] outline-none transition-colors"
                  placeholder="Answer"
                  required
                />
              </div>
        ...
        <div className="mt-12 flex justify-between">
          ...
          {step < formSteps.length - 1 ? (
            ...
            <button
              ...
              disabled={(currentStep.required && !formData[currentStep.id]) || isSubmitting || !captchaAnswer}
              className={`px-6 py-3 font-mono text-sm uppercase tracking-wider bg-[#fe0000] text-white rounded-full transition-colors flex items-center gap-2 ${((currentStep.required && !formData[currentStep.id]) || isSubmitting || !captchaAnswer) ? 'opacity-50 cursor-not-allowed' : 'hover:bg-red-700'}`}
            >
